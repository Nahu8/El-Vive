        ...
        this.apiService.getMinistryMedia(mid).subscribe({
          next: (data) => {
            this.ministryMediaState[mid] = {
              ...data,
              iconUrl: data.iconUrl ? `http://127.0.0.1:3000${data.iconUrl}` : null,
              cardImageUrl: data.cardImageUrl ? `http://127.0.0.1:3000${data.cardImageUrl}` : null,
              photos: (data.photos || []).map((p: any) => ({ ...p, url: `http://127.0.0.1:3000${p.url}` })),
              videos: (data.videos || []).map((v: any) => ({ ...v, url: `http://127.0.0.1:3000${v.url}` }))
            };
          },
          error: () => {}
        });
  ...
  onMinistryCardImageUpload(event: Event, ministryId: string): void {
    ...
    this.apiService.uploadMinistryCardImage(ministryId, file).subscribe({
      next: (res) => {
        ...
        this.ministryMediaState[ministryId].cardImageUrl = `http://127.0.0.1:3000${res.cardImageUrl}?t=${Date.now()}`;
  ...
  onMinistryIconUpload(event: Event, ministryId: string): void {
    ...
    this.apiService.uploadMinistryIcon(ministryId, file).subscribe({
      next: (res) => {
        ...
        this.ministryMediaState[ministryId].iconUrl = `http://127.0.0.1:3000${res.iconUrl}?t=${Date.now()}`;
    ...
    this.apiService.uploadMinistryPhoto(ministryId, file).subscribe({
      next: (res) => {
        ...
        this.ministryMediaState[ministryId].photos.push({
          id: res.photoId, name: res.imageName, url: `http://127.0.0.1:3000${res.photoUrl}?t=${Date.now()}`
        });
        this.uploadingMediaFor = null;
    ...
    this.apiService.uploadMinistryVideo(ministryId, file).subscribe({
      next: (res) => {
        ...
        this.ministryMediaState[ministryId].videos.push({
          id: res.videoId,
          name: res.videoName,
          url: `http://127.0.0.1:3000${res.videoUrl}?t=${Date.now()}`
        });
        this.uploadingMediaFor = null;
