...
export class AdminContactoComponent implements OnInit {
  ...
  pageContent: {
    hero: { title: string; subtitle: string; backgroundImageUrl?: string; backgroundVideoUrl?: string };
    ...
    hero: { title: 'CONTACTO', subtitle: 'Estamos aquí para servirte. No dudes en contactarnos.' },
    intro: { title: '', content: '' },
  ...
  onHeroImageSelected(event: Event): void {
    ...
    this.http.post<any>(`${this.apiBase}/api/media/upload`, fd).subscribe({
      next: (res) => {
        this.pageContent.hero.backgroundImageUrl = res.path || res.url || '';
