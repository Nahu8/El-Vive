...
export class MinisterioDetalleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.publicApi.getMinistriesConfig().subscribe({
      next: (configData) => {
        if (configData.pageContent?.backgroundColor) {
          this.backgroundColor = configData.pageContent.backgroundColor;
        }
      }
    });
      ...
      this.publicApi.getMinistryDetail(id).subscribe({
        next: (data) => {
          ...
          if (data.photos && data.photos.length > 0) {
            this.photoUrls = data.photos.map((p: any) => 'http://127.0.0.1:3000' + p.url);
          }
          if (data.iconUrl) {
            this.ministry.iconFullUrl = 'http://127.0.0.1:3000' + data.iconUrl;
          }
          ...
          this.internalVideoUrls = (data.internalVideos || []).map(
            (v: { url: string }) => 'http://127.0.0.1:3000' + v.url
          );
