    ...
    this.publicApi.getMinistriesConfig().subscribe({
      next: (data) => {
        ...
        if (data.hero) {
          ...
          this.heroImageUrlLight = data.hero.heroImageUrlLight ? this.publicApi.resolveAssetUrl(data.hero.heroImageUrlLight) : null;
          this.heroImageUrlDark = data.hero.heroImageUrlDark ? this.publicApi.resolveAssetUrl(data.hero.heroImageUrlDark) : null;
        ...
        if (data.ministries && Array.isArray(data.ministries)) {
          this.ministries = data.ministries
            ...
            .map((m: any) => ({
              ...
              icon: m.icon || '',
              iconUrl: m.iconUrl ? environment.apiBaseUrl + m.iconUrl : null,
              image: m.cardImageUrl ? environment.apiBaseUrl + m.cardImageUrl : (m.image || ''),
