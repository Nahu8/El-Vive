...
export class AdminDiasReunionComponent implements OnInit {
  ...
  heroImageLightPreview: string | null = null;
  ...
  loadAllData(): void {
    ...
    this.apiService.getMeetingDays().subscribe({
      next: (meetingDaysData) => {
        ...
        this.heroImageLightPreview = meetingDaysData.heroImageUrlLight ? this.apiService.resolveAssetUrl(meetingDaysData.heroImageUrlLight) + '?t=' + Date.now() : null;
        this.heroImageDarkPreview = meetingDaysData.heroImageUrlDark ? this.apiService.resolveAssetUrl(meetingDaysData.heroImageUrlDark) + '?t=' + Date.now() : null;
  ...
  onHeroImageSelected(event: Event): void {
    ...
    this.apiService.uploadMeetingDaysHeroImage(file).subscribe({
      next: (res) => {
        ...
        this.heroImagePreview = `${environment.apiBaseUrl}/api/meeting-days/hero-image?t=${Date.now()}`;
  ...
  onHeroThemeImageSelected(event: Event, mode: 'light' | 'dark'): void {
    ...
    this.apiService.uploadSectionIcon('meeting-days', sectionKey, file).subscribe({
      next: () => {
        const url = this.apiService.getSectionIconUrl('meeting-days', sectionKey) + '?t=' + Date.now();
        if (mode === 'light') this.heroImageLightPreview = url;
        else this.heroImageDarkPreview = url;
  ...
  clearHeroThemeImage(mode: 'light' | 'dark'): void {
    const sectionKey = mode === 'light' ? 'hero-light' : 'hero-dark';
    this.apiService.deleteSectionIcon('meeting-days', sectionKey).subscribe({
      next: () => {
        if (mode === 'light') this.heroImageLightPreview = null;
        else this.heroImageDarkPreview = null;
